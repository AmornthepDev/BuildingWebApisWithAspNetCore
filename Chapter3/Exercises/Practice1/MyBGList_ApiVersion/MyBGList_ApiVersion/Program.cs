...
builder.Services.AddCors(options => {
    ...
    options.AddPolicy(name: "AnyOrigin_GetOnly",
        cfg =>
        {
            cfg.AllowAnyOrigin().AllowAnyHeader().WithMethods("GET");
        });
});
...
.AddApiExplorer(options => // For Controller-based API?
{
...
builder.Services.AddEndpointsApiExplorer(); // For Minimal API? probably don't need since .NET8+ (not sure)
